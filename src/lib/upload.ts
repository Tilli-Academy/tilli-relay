...
export const UPLOAD_DIR = process.env.RELAY_UPLOAD_DIR || "/tmp/relay-uploads";
export const MAX_FILE_SIZE = parseInt(process.env.RELAY_MAX_UPLOAD_SIZE || "10485760", 10); // 10MB
...
export async function cleanupTempFiles(curlCommand: string): Promise<void> {
  const filePathRegex = /@(\/tmp\/relay-uploads\/[^\s;'"]+)/g;
  let match;
